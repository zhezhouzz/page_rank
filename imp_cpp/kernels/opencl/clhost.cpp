...
int KernelOpencl::make_cl_kernel(cl_program* program, const std::string& cl_kernel_filepath) {
    char* source_str = nullptr;
    size_t source_size = load_file(cl_kernel_filepath.c_str(), &source_str);
...
KernelOpencl::KernelOpencl() {
    ...
    CL_ERR_HANDLE;
    make_cl_kernel(&program,
                   "/Users/admin/workspace/page_rank/imp_cpp/kernels/opencl/page_rank_kernel.cl");
    CL_ERR_HANDLE;
    sparse_mxv_kernel = clCreateKernel(program, "sparse_mxv", &ret_code);
    CL_ERR_HANDLE;
    dense_mxv_kernel = clCreateKernel(program, "dense_mxv", &ret_code);
    CL_ERR_HANDLE;
...
int KernelOpencl::upload(taco_tensor_t* c_tensor_y, taco_tensor_t* c_tensor_alpha,
    ...
    tensor_x_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, tensor_x_length * sizeof(double),
                                  nullptr, &ret_code);
    CL_ERR_HANDLE;
    ...
    tensor_A2_pos_mem =
        clCreateBuffer(context, CL_MEM_READ_ONLY, A2_pos_length * sizeof(int), nullptr, &ret_code);
    CL_ERR_HANDLE;
    tensor_A2_coord_mem = clCreateBuffer(context, CL_MEM_READ_ONLY, A2_coord_length * sizeof(int),
                                         nullptr, &ret_code);
    CL_ERR_HANDLE;
    tensor_A_vals_mem = clCreateBuffer(context, CL_MEM_READ_ONLY, A_vals_length * sizeof(double),
                                       nullptr, &ret_code);
    CL_ERR_HANDLE;
    ret_code = clEnqueueWriteBuffer(command_queue, tensor_A2_pos_mem, CL_TRUE, 0,
                                    A2_pos_length * sizeof(int), A2_pos, 0, nullptr, nullptr);
    CL_ERR_HANDLE;
    ret_code = clEnqueueWriteBuffer(command_queue, tensor_A2_coord_mem, CL_TRUE, 0,
                                    A2_coord_length * sizeof(int), A2_coord, 0, nullptr, nullptr);
    CL_ERR_HANDLE;
    ret_code = clEnqueueWriteBuffer(command_queue, tensor_A_vals_mem, CL_TRUE, 0,
    ...
    tensor_y_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, tensor_y_length * sizeof(double),
                                  nullptr, &ret_code);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(sparse_mxv_kernel, 0, sizeof(uint32_t), &tensor_x_length);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(sparse_mxv_kernel, 1, sizeof(cl_mem), &tensor_A2_pos_mem);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(sparse_mxv_kernel, 2, sizeof(cl_mem), &tensor_A2_coord_mem);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(sparse_mxv_kernel, 3, sizeof(cl_mem), &tensor_A_vals_mem);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(sparse_mxv_kernel, 4, sizeof(double) * CL_WORK_GROUP, nullptr);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(sparse_mxv_kernel, 7, sizeof(cl_mem), &tensor_z_mem);
    CL_ERR_HANDLE;
    ret_code =
        clSetKernelArg(sparse_mxv_kernel, 8, sizeof(cl_mem), (double*)(c_tensor_alpha->vals));
    CL_ERR_HANDLE;
    return 0;
}
...
int KernelOpencl::page_rank_once(bool flag_x2y) {
    ...
    if (flag_x2y) {
        ret_code = clSetKernelArg(sparse_mxv_kernel, 5, sizeof(cl_mem), &tensor_x_mem);
        CL_ERR_HANDLE;
        ret_code = clSetKernelArg(sparse_mxv_kernel, 6, sizeof(cl_mem), &tensor_y_mem);
        CL_ERR_HANDLE;
    } else {
        ret_code = clSetKernelArg(sparse_mxv_kernel, 6, sizeof(cl_mem), &tensor_x_mem);
        CL_ERR_HANDLE;
        ret_code = clSetKernelArg(sparse_mxv_kernel, 5, sizeof(cl_mem), &tensor_y_mem);
        CL_ERR_HANDLE;
    }
    ret_code = clEnqueueNDRangeKernel(command_queue, sparse_mxv_kernel, 1, nullptr,
...
int KernelOpencl::upload_dense_mxv(taco_tensor_t* c_tensor_y, taco_tensor_t* c_tensor_alpha,
    ...
    double* __restrict A_vals = (double*)(c_tensor_A->vals);
    tensor_A_vals_mem =
        clCreateBuffer(context, CL_MEM_READ_ONLY, A1_dimension * A2_dimension * sizeof(double),
                       nullptr, &ret_code);
    CL_ERR_HANDLE;
    ret_code = clEnqueueWriteBuffer(command_queue, tensor_A_vals_mem, CL_TRUE, 0,
                                    A1_dimension * A2_dimension * sizeof(double), A_vals, 0,
                                    nullptr, nullptr);
    CL_ERR_HANDLE;
    ...
    tensor_y_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, tensor_y_length * sizeof(double),
                                  nullptr, &ret_code);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(dense_mxv_kernel, 0, sizeof(uint32_t), &tensor_x_length);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(dense_mxv_kernel, 1, sizeof(cl_mem), &tensor_A_vals_mem);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(dense_mxv_kernel, 2, sizeof(double) * CL_WORK_GROUP, nullptr);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(dense_mxv_kernel, 5, sizeof(cl_mem), &tensor_z_mem);
    CL_ERR_HANDLE;
    ret_code = clSetKernelArg(dense_mxv_kernel, 6, sizeof(cl_mem), (double*)(c_tensor_alpha->vals));
    CL_ERR_HANDLE;
    return 0;
}
int KernelOpencl::dense_mxv(bool flag_x2y) {
    ...
    size_t local_work_size[] = {CL_WORK_GROUP};
    if (flag_x2y) {
        ret_code = clSetKernelArg(dense_mxv_kernel, 3, sizeof(cl_mem), &tensor_x_mem);
        CL_ERR_HANDLE;
        ret_code = clSetKernelArg(dense_mxv_kernel, 4, sizeof(cl_mem), &tensor_y_mem);
        CL_ERR_HANDLE;
    } else {
        ret_code = clSetKernelArg(dense_mxv_kernel, 4, sizeof(cl_mem), &tensor_x_mem);
        CL_ERR_HANDLE;
        ret_code = clSetKernelArg(dense_mxv_kernel, 3, sizeof(cl_mem), &tensor_y_mem);
        CL_ERR_HANDLE;
    }
    ret_code = clEnqueueNDRangeKernel(command_queue, dense_mxv_kernel, 1, nullptr, global_work_size,
...
KernelOpencl::~KernelOpencl() {
    ret_code = clReleaseKernel(sparse_mxv_kernel);
    CL_ERR_HANDLE;
