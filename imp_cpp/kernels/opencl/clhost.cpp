...
int prepare() {
    ...
    size_t source_size =
        load_file("/Users/admin/workspace/page_rank/imp_cpp/kernels/opencl/page_rank_kernel.cl",
                  &source_str);
...
int download(bool flag_x2y, taco_tensor_t** c_tensor_x, taco_tensor_t** c_tensor_y) {
    ...
    if (flag_x2y) {
        clEnqueueReadBuffer(::command_queue, ::tensor_x_mem, CL_TRUE, 0,
                            ::tensor_x_length * sizeof(double), ::y->vals, 0, nullptr,
                            nullptr);
        CL_ERR_HANDLE;
        clEnqueueReadBuffer(::command_queue, ::tensor_y_mem, CL_TRUE, 0,
                            ::tensor_x_length * sizeof(double), ::x->vals, 0, nullptr,
                            nullptr);
        CL_ERR_HANDLE;
    } else {
        clEnqueueReadBuffer(::command_queue, ::tensor_y_mem, CL_TRUE, 0,
